...
echo Erasing chip
if not test -e flash_erase.bat ;echo NOT FOUND flash_erase.bat
if not test -e flash_erase.bat ;exit -1
./flash_erase.bat.fish
...
./update_version.bat.fish

./clean_compile_two_versions.bat.fish
if not test -e deliver/rusefi_release.hex ;echo FAILED RELEASE
if not test -e deliver/rusefi_release.hex ;exit -1

if not test -e deliver/rusefi_debug.hex ;echo FAILED DEBUG
if not test -e deliver/rusefi_debug.hex ;exit -1
...
if not test -e java_console_binary/rusefi_console.jar ;echo CONSOLE COMPILATION FAILED
if not test -e java_console_binary/rusefi_console.jar ;exit -1
...
if not test -e build/rusefi_simulator.exe ;echo SIMULATOR COMPILATION FAILED
if not test -e build/rusefi_simulator.exe exit -1
...
if not test -e ../rusefi_console.zip ;echo CONSOLE ZIP FAILED
if not test -e ../rusefi_console.zip ;exit -1
...
cd temp
./winscp.com /script=../ftp_commands.txt.fish
